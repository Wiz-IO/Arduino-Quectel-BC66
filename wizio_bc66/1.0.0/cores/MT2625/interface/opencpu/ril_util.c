...
char* Ql_RIL_FindLine(char *line, u32 len,char *str)
{
    ...
    pStr = Ql_MEM_Alloc(str_len + 4 + 1);
    if (NULL == pStr)
         return NULL;
    
    if (len >= str_len + 4)//two \r\n
    {
        p = line;
        Ql_memset(pStr, 0, str_len + 5);
        Ql_sprintf(pStr,"\r\n%s\r\n",str);
        for (i = 0;i < len - (str_len + 4) + 1; i++)
        {
            if (0 == Ql_strncmp(p, pStr, str_len + 4))
            {
                Ql_MEM_Free(pStr);
                return p;
            }else{
    ...
    if (len >= str_len + 2)//two \r or two\n
    {
        ...
        // <LF>xx<LF>
        pStr2 = (char*)Ql_MEM_Alloc(str_len + 5);
        Ql_memset(pStr2, 0, str_len + 5);
        Ql_sprintf(pStr2,"\n%s\n",str);

        // xx<CR><LF>
        pStr3 = (char*)Ql_MEM_Alloc(str_len + 5);
        Ql_memset(pStr3, 0, str_len + 5);
        Ql_sprintf(pStr3,"%s\r\n",str);

        for (i = 0;i < len - (str_len + 2) + 1; i++)
        {
            if ((0 == Ql_strncmp (p, pStr, str_len + 2)) ||
                (0 == Ql_strncmp (p, pStr2, str_len + 2)) ||
                (0 == Ql_strncmp (p, pStr3, str_len + 2)))
            {
                Ql_MEM_Free(pStr);
                Ql_MEM_Free(pStr2);
                Ql_MEM_Free(pStr3);
